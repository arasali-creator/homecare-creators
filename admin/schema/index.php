...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header('Location: /admin/schema/' . ($pg ? '?path='.urlencode($pg) : ''));
    exit;
}

$existing = $path ? hc_all("SELECT * FROM hc_page_schema WHERE page_path=? ORDER BY schema_type", [$path]) : [];
$page_name = '';
  ...
  <div class="table-wrap">
    <table>
      ...
      <tr>
        ...
        <td class="td-actions">
          <form method="POST" style="display:inline"><input type="hidden" name="action" value="toggle"><input type="hidden" name="id" value="<?= $s['id'] ?>"><input type="hidden" name="page_path" value="<?= h($path) ?>"><button class="btn btn-ghost btn-sm"><?= $s['active']?'Disable':'Enable' ?></button></form>
          <form method="POST" style="display:inline"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= $s['id'] ?>"><input type="hidden" name="page_path" value="<?= h($path) ?>"><button class="btn btn-ghost btn-sm danger" data-confirm="Delete this schema block?">Delete</button></form>
        </td>
      </tr>
      <?php endforeach ?>
      </tbody>
...
<div class="card">
  ...
  <div id="tab-faq" class="tab-panel active">
    <form method="POST">
      ...
      <input type="hidden" name="page_path" value="<?= h($path) ?>">
